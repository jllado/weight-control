...
- Every new exercise must include a reviewed picture matching the existing illustration style; follow `docs/exercises/pictures.md` and verify the saved picture before considering any catalog addition complete.
...
- Finish implementation before validation; use focused checks during development and rerun failing checks first after fixes.
- Keep checks sequential and wait for process exit, including cleanup; an occupied lock means wait for the existing run, not restart or kill it. The validated release gate coordinates frontend/backend concurrency and two browser workers inside one locked invocation; its `sequential` mode is the fallback. Never launch independent checks concurrently.
- For releases, define focused validation from the changed areas, pass it before the candidate commit, and build deployable artifacts before pushing or deploying. Run the complete release-artifact gate only for shared infrastructure, authentication or authorization, shared data contracts, dependency/toolchain, build/deployment/PWA changes, explicit broad-validation plans, or unclear risk.
