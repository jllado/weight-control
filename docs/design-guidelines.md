...
- Compact actions are rectangular outlined primary-blue buttons; Delete/Remove is outlined danger-red. Routine actions preserve their established colors: Complete/Edit is outlined success-green and Undo/Delete is outlined warning-amber. Do not use round, filled, borderless, or labeled variants for the same compact action.
...
- Workout editor add actions are the explicit labeled-group exception: keep Add warm-up, Add exercise, Add stretching, and Add stretching set in one full-width vertical column at every viewport.
