...
| HTML/assets | public/index.html uses Vue CLI template BASE_URL; public icons, manifest and push worker retain root URLs. Caddyfile.j2 uses an explicit static-path allow-list before its SPA rewrite; it now includes `/third-party-notices.txt`. Vite must preserve history fallback and update this allow-list for new asset paths. |
