...
Nutrition → Foods manages a separate user-owned food catalog through session-authenticated `/api/foods` CRUD. Import unique meal foods once using the latest meal date, meal ID, and food position; match trimmed names case-insensitively. Manual and confirmed Coach meal saves register new names transactionally without overwriting catalog edits. Deletion and renaming suppress automatic recreation of old names; explicit Add can restore them. Catalog edits affect future food-picker reuse only, preserving saved meal and recipe snapshots. Recipe-only ingredients enter the catalog when saved in a meal; deleting meals does not remove catalog entries. No new Coach domain or Action is exposed, and catalog entries are excluded from consumption totals and reflection inputs.
