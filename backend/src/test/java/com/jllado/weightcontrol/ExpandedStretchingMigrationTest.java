...
@Testcontainers
class ExpandedStretchingMigrationTest {
    @Container private static final MariaDBContainer<?> DATABASE = new MariaDBContainer<>("mariadb:11.8").withDatabaseName("expanded_stretching");

    @BeforeEach void preparePreviousCatalog() { flyway("63").clean(); flyway("63").migrate(); }

    @Test void expandsToTwentyTimedStretchesWithReadablePictures() throws Exception {
        flyway("64").migrate();
        ...
        assertEquals(0, flyway("64").migrate().migrationsExecuted);
    }

    @Test void preservesAnExistingMatchingExerciseAndItsCustomPicture() throws Exception {
        try (var connection = DATABASE.createConnection(""); var statement = connection.createStatement()) {
            statement.executeUpdate("INSERT INTO exercises (name, description, tracking_mode, custom_image_path) VALUES ('SEATED BUTTERFLY STRETCH', 'My own exercise', 'REPS', 'exercise-images/custom.jpg')");
        ...
        flyway("64").migrate();
        try (var connection = DATABASE.createConnection(""); var statement = connection.createStatement()) {
            try (var rows = statement.executeQuery("SELECT description, tracking_mode, exercise_type, built_in_image_key, custom_image_path FROM exercises WHERE lower(name) = 'seated butterfly stretch'")) {
            ...
            try (var rows = statement.executeQuery("SELECT count(*) FROM exercises WHERE exercise_type = 'STRETCHING'")) { assertTrue(rows.next()); assertEquals(19, rows.getInt(1)); }
        }
    }
